/**
 * site-alt.ts — content object for the ALTERNATE homepage only.
 *
 * This is a fork of content/site.ts. It keeps every existing field intact
 * (nothing here breaks components that still import the original
 * `content/site`) and adds/updates only what the alternate homepage needs:
 *
 *   - hero.introduction   → merged into a single paragraph (intro + experience)
 *   - hero.experience     → REMOVED (folded into hero.introduction)
 *   - hero.ctas           → updated primary/secondary labels
 *   - hero.flagshipTeaser → reshaped to { text, href } for the quiet mono teaser
 *   - auditFramework      → NEW section content
 *   - proofBar            → NEW stat-strip content
 *
 * Do not rename this file to `site.ts` yet — see IMPLEMENTATION-NOTES.md for
 * the cutover plan. Components on the alternate homepage import from
 * "@/content/site-alt"; everything else on the live site continues to import
 * from "@/content/site" unchanged.
 */
...
export const site = {
  ...
  hero: {
    ...
    // Single body paragraph — merges the old `introduction` + `experience`
    // fields into one. The `experience` field has been removed; do not
    // reintroduce a second body paragraph on the alternate homepage.
    introduction:
      "I build production AI systems that connect voice agents, automation, CRM platforms, and human teams into measurable revenue engines. 25 years in marketing — the last 15+ building the systems businesses actually run on.",
    ctas: {
      primary: { label: "View the Systems", href: "#architecture" },
      secondary: { label: "Recruiter Fast Path", href: "/recruiter" },
    },
    // Quiet, low-emphasis teaser rendered as font-mono text-xs below the CTA
    // row — not a third CTA. Links to the flagship case study.
    flagshipTeaser: {
      text: "Flagship: The multi-agent system that runs my own company",
      href: "/seinfeld-hq",
    },
  },
  ...
  // NEW — the reward-engineering / rival-auditor / Tiger Team content block.
  // Powers components/sections/AuditFramework.tsx.
  auditFramework: {
    eyebrow: "AI Evaluation & Accountability",
    heading: "A system builder who pressure-tests AI before it ships.",
    body: "Three independent auditors attack the same workflow from different labs. A reward engine filters weak or fabricated claims. Confirmed findings come out as ranked, ready-to-run fixes — not vague AI opinions.",
    bullets: [
      "Reward engineering makes AI fight for truth instead of gaming the metric.",
      "Rival auditors from different labs collide to surface failure modes most teams never see.",
      "Cross-verified findings become ranked fixes — accountable AI, not black-box output.",
    ],
    cta: {
  ...
  // NEW — powers components/sections/ProofBar.tsx.
  proofBar: [
    {
      value: "$750K+/mo",
      label: "attributed pipeline from AI-driven lifecycle automation",
    },
    {
      value: "$90K+/yr saved",
      label: "Zapier-to-n8n migration, zero downtime",
    },
    {
      value: "21×",
      label: "faster lead qualification vs. manual response",
    },
  ],
