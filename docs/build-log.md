...
## 2026-07-09 (evening) — Demand-sweep fleet: 25 agents blanket the AI-voice-agent market in 13 minutes

**The move [DECK]:** Matt asked where market research belongs — Claude Code CLI or the connector-rich desktop session. The answer reframed the question: the bottleneck was *data access, not intelligence* (Apify/Semrush/ZoomInfo/Sheets connectors live here, not in the CLI). Then: "spawn as many bots as you need. Let's blanket it." One Workflow: 12 parallel source scouts (3 Reddit vertical clusters, niche forums, Indeed, Craigslist, Upwork/freelance, Bark/Thumbtack/Angi, 7 competitor sites, Semrush, ZoomInfo intent, voice-of-customer reviews, X/LinkedIn/GHL communities) → 9 skeptic verification batches fact-checking every citation → 3 synthesis agents (vertical scorecard, GTM memo, coverage critic). **25 agents, 502 tool calls, 13m07s, zero failures → 203 deduped, verified demand signals across 16 verticals.** Matt, watching the live progress tree: "I love that I can watch that little monitor."

**Findings:** law firms (24 signals), HVAC (15), dental (14), property management (13), plumbing (10) top the scorecard. **Fastest beachhead = HVAC** — a substitution sale against the $300/mo human answering services owner-operators already pay for and resent. 42 high-urgency signals. Honest-framing beat: roofing returned a genuine *negative* signal (referral-driven trade; roofers say missed calls don't cost jobs) — the sweep is allowed to kill a hunch, and did.

**Agent workaround beats [DECK]:** Reddit hard-blocks Anthropic's fetcher — scouts independently found legitimate routes (Playwright on public old.reddit pages, PullPush public archive) without bypassing anything. Blocked surfaces (VerticalScope forums behind a Tollbit 402 paywall, Dentaltown login wall, Yelp/Angi/Quora/X 403s) were skipped per rules and *reported* — the memo's coverage appendix names every block and the exact re-run once unblocked. Zero fabricated citations survived: 59 rows live-verified, 0 suspect.

**Blockers surfaced (tech-debt list):** Zapier Sheets auth stale (one-click re-auth link handed to Matt); all 6 Supabase projects paused, restore correctly denied by permission guardrails pending Matt's say-so (SQL fully staged for a 30-second finish); Apify token invalid; Semrush plan lacks MCP access; ZoomInfo intent topics not entitled. The quantitative layer (search volume + buyer intent) is the known gap — everything qualitative is in hand.

Also this session: Google Calendar connector verified live (GMS Appointments = 30-min lead-call calendar) and saved to memory — the booking loop is ready the day outreach starts.

**Update, same evening [DECK] — the Zapier blocker died on Matt's own rails:** Matt: "We changed from Zapier. What did we go to that was free?" → n8n. Instead of the Zapier re-auth, built a 5-node n8n workflow ("Demand Sweep → Google Sheets Import") through the n8n MCP: webhook → branch → Google Sheets append / raw Sheets-API proxy, using the Google credential already living in his instance (refreshed that same day). Activated it, POSTed the 203 rows + scorecard + memo through it in 11 chunked calls, read back cell counts to verify (204/17/83), deactivated the webhook, kept the workflow as a reusable bulk-loader. Zero clicks from Matt, zero new credentials, $0. The pattern: when a paid connector goes stale, the client's own automation stack IS the connector.

---

## 2026-07-10 — Blueprint Train #2: the field-guide series [TRAIN]

Second WorkflowCarousel shipped directly under the Agentic section's original train, seeded from the 9 guide PDFs (93 pages OCR'd via Vision). Full pipeline ran hands-off over Desktop Commander: PDFs pulled from Desktop/Downloads into `public/workflows/` (untracked), pages OCR'd + contact-sheeted, 15 elements curated (no covers, one slide per element — tigsen deck was a page-identical dupe of Paying Agents, both voice guides identical, tiger 5-pager condensed dupes all skipped), rendered 1600px webp q82 (60–164KB). Arc: 698-incidents problem → Tiger/Sentinel two-winners → reward number line → gauntlet → cross-lab rivalry → voice end-to-end → six phases → eight layers → integrations → three drifts → promotion gate → monitoring stack → nine security controls → board-deck outcomes → timeline gates. Wired as `agentic.guides` in `content/site.ts`; `WorkflowCarousel` already took an `items` prop so the section just renders a second instance with a mono eyebrow ("The field-guide series — problem to roadmap"). Gitignore now guards raw sources (*.pdf/png/jpg in workflows/). tsc clean.
